...
**claude-workflow | Phase 1 | review (Codex late finding)** — Cross-board integrity is not enforced at the schema level. `placements` allows a version_id from board A with an entry_id from board B. Same for `accumulated_scores` (board_id + entry_id mismatch) and `board_references` (pinned_version_id from a different board). This is valid but harmless in Phase 1 (no write paths). Phase 2 MUST address this — options: (a) application-level validation in CRUD handlers, (b) composite FKs with redundant board_id columns, (c) triggers. Option (a) is simplest and most common; option (b) is the most robust. Decide at Phase 2 start.
